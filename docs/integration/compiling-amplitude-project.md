...
As explained in the [project architecture](../project/project-architecture.md) documentation, an Amplitude project is mainly based on JSON files. But when it's time to integrate the engine with your game, it's required to compile these JSON files into binaries, through the `flatc` compiler.
...
|--|---|--|
| `-f`, `--flatc`| `false` | The path to the `flatc` executable. This argument is optional and if not provided, the script will look up for a valid `flatc` executable in the `PATH` environment variable and in the `bin` directory of your Amplitude Audio SDK installation. |
| `-p`, `--project-path` | `true` | The path to the Amplitude project. You should specify the full path to the directory containing Amplitude assets. |
| `-b`, `--build-path` | `true` | The path to the directory where built files will be created. The build directory structure will be the same as the project directory. If the given path doesn't exist it will be automatically created. |
...
Both `build_project.py` and `clean_project.py` scripts provide the following additional arguments:

| Argument | Required | Description |
|--|---|--|
| `-h`, `--help` | `false` | Displays help/documentation about the usage of the running script. |
| `--no-logo` | `false` | Used to not display the copyright header. |
| `-v`, `--version` | `false` | Shows the version of the running script. |
